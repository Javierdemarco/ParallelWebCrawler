...
public final class CrawlerConfiguration {
  ...
  private CrawlerConfiguration(List<String> startPages,
                               List<Pattern> ignoredUrls,
                               List<Pattern> ignoredWords, int parallelism,
                               String implementationOverride, int maxDepth,
                               Duration timeout, int popularWordCount,
                               String profileOutputPath, String resultPath) {
  ...
  /**
   * An unmodifiable {@link List} of URLs that define the starting points of the
   * web crawl. It should not contain any duplicate URLs.
   */
  public List<String> getStartPages() { return startPages; }

  /**
   * A {@link List} of regular expression {@link Pattern}s that determine which
   * URLs, if any, the web crawler should not follow.
   */
  public List<Pattern> getIgnoredUrls() { return ignoredUrls; }

  /**
   * A {@link List} of regular expression {@link Pattern}s that determine which
   * words, if any, the web crawler should not be counted toward the popular
   * word count returned from the crawl.
   */
  public List<Pattern> getIgnoredWords() { return ignoredWords; }

  /**
   * The desired parallelism (e.g., number of CPU cores) that should be used for
   * the web crawl. This setting is optional.
   *
   * <p>If set to 1, the legacy sequential crawler will be used (unless
   * {@link #getImplementationOverride()} explicitly specifies otherwise). If
   * set to a value less than 1, the crawler will default to using the number of
   * available CPU cores on the system.
   */
  public int getParallelism() { return parallelism; }

  /**
   * An explicit override for which web crawler implementation should be used
   * for this crawl.
   *
   ...
   *
   * <p>If unset or empty, the value of {@link #getParallelism()} setting is
   * used to determine which crawler implementation to use.
   */
   ...
   *
   * <p>The "depth" of a crawl is the number of links the crawler has followed,
   * starting from the starting URLs (see {@link #getStartPages()}).
   *
   * <p>
   * <p>Example:
   *
   * <p>Suppose the max depth is 2, and suppose the starting page "A" links to
   ...
   * </pre>
   *
   * <p>In this example, the crawler will visit at most A, B, C, and D. The
   * crawler may visit <i>fewer</i> pages if it runs out of time before it can
   * finish. See {@link #getTimeout()}.
   */
   ...
   *
   * <p>This is not a strict deadline, meaning the crawler does not have to drop
   * everything and terminate immediately. However, Once this amount of time has
   * been reached, the crawler will finish processing any HTML it has already
   * downloaded, but it will not download any more pages or follow any more
   * links.
   */
   ...
   *
   * <p>See {@link com.udacity.webcrawler.json.CrawlResult#getWordCounts()} for
   * more information.
   */
  public int getPopularWordCount() { return popularWordCount; }

  /**
   * Path to the output file where performance data from this web crawl should
   * be written.
   *
   * <p>If a file already exists at the path, performance information should be
  ...
  /**
   * Path to the output file where the result data from this web crawl should be
   * written.
   *
   * <p>If a file already exists at the path, the existing file should be
   * replaced.
   *
   * <p>If the path is empty, the data will be written to standard output.
   *
  ...
  public static final class Builder {
     ...
     *
     * <p>Does nothing if the given page has already been added. See {@link
     * #getStartPages()}.
     */
    ...
    /**
     * Adds a regular expression pattern that defines URLs to ignore during the
     * crawl.
     *
     * <p>Does nothing if the same pattern has already been added. See {@link
     * #getIgnoredUrls()}.
     *
     * @param patterns one or more regular expressions that define a valid
     *     {@link Pattern}.
     */
    ...
    /**
     * Adds a regular expression pattern that defines words to ignore when
     * computing the popular counts.
     *
     * <p>Does nothing if the same pattern has already been added. See {@link
     * #getIgnoredWords()}.
     *
     * <p>See {@link com.udacity.webcrawler.json.CrawlResult#getWordCounts()}
     * for more information about the popular word computation.
     *
     * @param patterns one or more regular expressions that define a valid
     *     {@link Pattern}.
     */
    ...
    /**
     * Overrides the {@link com.udacity.webcrawler.WebCrawler} implementation
     * that should be used for the crawl.
     *
    ...
    public Builder setImplementationOverride(String implementationOverride) {
      this.implementationOverride =
          Objects.requireNonNull(implementationOverride);
      return this;
    }
    ...
    /**
     * Sets the maximum amount of time allowed for the crawl, specified in
     * seconds.
     *
    ...
    /**
     * Sets the number of most popular words that should be reported by the
     * crawl.
     *
    ...
    /**
     * Sets the path to the file where profiling data for this crawl should be
     * written.
     *
    ...
    /**
     * Sets the path to the file where the result of this crawl should be
     * written.
     *
    ...
    public CrawlerConfiguration build() {
      ...
      return new CrawlerConfiguration(
          ...
          ignoredWords.stream()
              .map(Pattern::compile)
              .collect(Collectors.toUnmodifiableList()),
          parallelism, implementationOverride, maxDepth,
          Duration.ofSeconds(timeoutSeconds), popularWordCount,
          profileOutputPath, resultPath);
    }
  }
}
